...
function typeToFsharp(type: Type): string {
  ...
  if (isUnionType(type)) return (type as any).name || "obj";
  if (type.kind === "Model") return (type as Model).name || "obj";
  return "obj";
}
...
function defaultValue(type: Type): string {
  ...
  if (type.kind === "Model" && (type as Model).name) return `Unchecked.defaultof<${(type as Model).name}>`;
...
function writeExpr(expr: string, type: Type, w: string): string {
  ...
  if (isUnionType(type) && (type as any).name)
    return `${(type as any).name}Methods.write ${w} ${expr}`;
  if (type.kind === "Model" && (type as Model).name)
    return `${(type as Model).name}Methods.write ${w} ${expr}`;
...
function readExpr(type: Type, r: string, optional?: boolean): string {
  ...
  if (isUnionType(type) && (type as any).name) {
    const decodeCall = `${(type as any).name}Methods.decode ${r}`;
    if (optional) return `if ${r}.IsNull() then (${r}.ReadNull(); None) else Some(${decodeCall})`;
    return decodeCall;
  }
  if (type.kind === "Model" && (type as Model).name) {
    const decodeCall = `${(type as Model).name}Methods.decode ${r}`;
    if (optional)
